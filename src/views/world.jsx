...
export const World = () => {
  ...
  return <div className="char">
    ...
    {store?.char && <div>
      ...
      <div onClick={() => setActiveStats(STATS_TAB_TYPES.SECONDARY)} className={(activeStats === STATS_TAB_TYPES.SECONDARY ? 'text-blue-500 ' : '') + 'inline-block pr-2 pl-2 border-black border-solid border-r border-l cursor-pointer'}>Locations</div>
      <div onClick={() => setActiveStats(STATS_TAB_TYPES.SKILLS)} className={(activeStats === STATS_TAB_TYPES.SKILLS ? 'text-blue-500 ' : '') + 'inline-block pr-2 pl-2 border-black border-solid border-r cursor-pointer'}>NPC</div>
      <div onClick={() => setActiveStats(STATS_TAB_TYPES.INVENTORY)} className={(activeStats === STATS_TAB_TYPES.INVENTORY ? 'text-blue-500 ' : '') + 'inline-block pr-2 pl-2 border-black border-solid border-r cursor-pointer'}>Vendors</div>
    </div>}
    <br />
