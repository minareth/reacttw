...
export const Inventory = (props) => {
  ...
  return inventory ? <div>
    {Object.keys(inventory).map((item) => {
      ...
      <ChevronDoubleDownIcon onClick={() => upgrade(char.id, 'skills', item)} className='h-5 inline-block text-yellow-700 ml-5 mb-2 cursor-pointer' />
    </div>})}
    <br />
    <div className="cursor-pointer">Add Item</div>
  </div> : <></>;
}
