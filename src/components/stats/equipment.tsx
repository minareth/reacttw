...
export const Equipment = (props) => {
  ...
  return equipment ? <div>
    {Object.keys(equipment).map((item) => {
      ...
      <ChevronDoubleUpIcon onClick={() => upgrade(char.id, 'skills', item)} className='h-5 inline-block text-blue-700 ml-5 mb-2 cursor-pointer' />
    </div>})}
    <br />
    <div className="cursor-pointer">Add Equipment</div>
  </div> : <></>;
}
